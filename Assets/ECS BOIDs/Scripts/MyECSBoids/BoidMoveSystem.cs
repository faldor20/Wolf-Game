...
namespace Samples.Boids
{
    public class BoidMovementSystem : JobComponentSystem
    {
        ...
        struct Steer : IJobProcessComponentData<Position, Heading>
        {
            ...
            [ReadOnly] public MainBoid settings;
            ...
            public void Execute ( [ReadOnly] ref Position position, ref Heading heading)
            {
                ...
                var normalHeading = math.normalizesafe (alignmentResult + separationResult + targetHeading + avoidObstacleHeading);
                //This means that the entity will disregard other forces when fleeing. something i wnat to remove.

                var targetForward = normalHeading; //math.select(normalHeading, avoidObstacleHeading,   < 0);
                var nextHeading = math.normalizesafe (forward + dt * (targetForward - forward));

                heading = new Heading { Value = nextHeading };
            }
        }
        float2 Alignment2D (float2 currentHeading, float2 groupHeading, float weight, )
        {

        }
        bool CheckBoidCollision2D (float2 centerPosition, float2 heading, float2 targetPosition, float colliderRadius, float viewangle)
        {
            if (math.distance (centerPosition, targetPosition) > colliderRadius) return false;
            if (viewangle != 0 && viewangle < 360)
            {
                // the viewangle is divided by two becuase we are only checking the angle on one side
                if (Vector2.Angle (centerPosition, heading) < viewangle / 2) return true;
            }
            return false;
        }
